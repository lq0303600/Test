...
开始 → 教师登录系统 → 进入作业管理页面 → 点击"发布作业"按钮 → 填写作业信息（作业标题、所属课程、作业内容、截止时间） → 点击确认发布 → 系统保存作业数据 → 显示发布成功提示 → 作业列表更新显示 → 学生可查看作业 → 结束

2. 考勤管理流程
--------------

开始 → 教师登录系统 → 进入考勤管理页面 → 点击"开始考勤"按钮 → 填写考勤信息（选择科目、选择日期、设置开始时间、设置结束时间） → 点击确认开始 → 系统创建考勤记录 → 考勤进行中 → 记录学生出勤状态（出勤、请假、缺勤、重点标记） → 点击"结束考勤" → 系统统计出勤数据 → 显示考勤结果 → 结束

3. AI智能问答流程
-----------------

开始 → 用户进入AI帮助页面 → 输入问题内容 → 点击发送 → 系统调用DeepSeek API（API密钥: sk-4a8795380a3b40bfb723383c0cb2fe66） → AI处理问题 → 返回智能回答 → 显示回答内容 → 显示当前时间 → 结束

4. 成绩录入流程
--------------

开始 → 教师登录系统 → 进入成绩录入页面 → 选择课程 → 系统加载学生名单 → 逐个输入学生成绩 → 点击保存成绩 → 系统验证数据 → 保存到数据库 → 显示保存成功 → 可查看成绩统计 → 结束
...
User（用户类）
- 属性：id, username, password, name, department, createdAt
- 方法：login(), register(), updateProfile()

Course（课程类）
- 属性：id, name, semester, teacherId, schedule, location, description
- 方法：getDetails(), getStudents()

Homework（作业类）
- 属性：id, title, courseId, content, publisherId, publishTime, deadline, status, submitCount
- 方法：publish(), end(), getSubmitList()

Attendance（考勤类）
- 属性：id, courseId, date, startTime, endTime, publisherId, presentCount, leaveCount, absentCount, markedStudents
- 方法：start(), end(), markStudent()

Student（学生类）
- 属性：id, studentId, name, className, contact
- 方法：getDetails(), getGrades()

Grade（成绩类）
- 属性：id, studentId, courseId, score, inputTime
- 方法：save(), getStatistics()

Book（图书类）
- 属性：id, title, author, isbn, publisher, location, status
- 方法：search(), borrow()

BorrowRecord（借阅记录类）
- 属性：id, userId, bookId, borrowDate, dueDate, returnDate, renewCount
- 方法：borrow(), renew(), return()

RepairRequest（报修请求类）
- 属性：id, content, location, userId, requestTime, repairmanId, status
- 方法：submit(), updateStatus()

Building（建筑物类）
- 属性：id, name, type, x, y, floors, description
- 方法：getDetails()

类关系：
- User 与 Course：一对多关系（teaches）
- User 与 Homework：一对多关系（publishes）
- User 与 Attendance：一对多关系（manages）
- Course 与 Homework：一对多关系（has）
- Course 与 Attendance：一对多关系（has）
- Course 与 Student：一对多关系（contains）
- Student 与 Grade：一对多关系（has）
- User 与 BorrowRecord：一对多关系（borrows）
- Book 与 BorrowRecord：一对多关系（borrowed）
- User 与 RepairRequest：一对多关系（requests）
